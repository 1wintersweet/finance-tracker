class UserStocksController < ApplicationController
  ...
  #handle user adding a stock to the portfolio in different cases
  def create
    if params[:stock_id].present? # stock  exist in our db
      @user_stock = UserStock.new(user_stock_params)
    else # stock doesn't exist in our db
      stock = Stock.find_by_ticker(params[:stock_ticker])
      if stock #stock valid
        @user_stock = UserStock.new(user: current_user, stock: stock)
      else # stock didn't return anything valid
        stock = Stock.new_from_lookup(params[:stock_ticker])
        if stock.save # if the stock is saved successfully valid stock, create the new association.
          @user_stock = UserStock.new(user:current_user, stock: stock)
        else
          @user_stock = nil
